                        ...
                        <div class="card">
                            <div class="card-body">
                                <h4 class="header-title mb-5">
                                    Fact Index :) Click here for create -
                                    <a class="{{ $indexFact->count() < 1 ? 'btn btn-success' : "text-danger btn disabled" }}" href="{{ route('fact.create') }}">{{ $indexFact->count() < 1 ? 'Create' : "You have permission to create a ITEM" }}</a>
                                </h4>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-dark mb-0">
                                        ...
                                        <tbody>
                                            @foreach($indexFact as $data)
                                                <tr>
                                                    <td>{{ $data->total }}</td>
                                                    <td>{{ $data->name }}</td>
                                                    <td>
                                                        <a href="{{ route('fact.edit', $data->id) }}" class="btn btn-outline-info btn-sm"><i class="fas fa-edit"></i></a>
                                                        <form action="{{ route('fact.destroy', $data->id) }}" method="post">
                                                            @csrf
                                                            @method('DELETE')
